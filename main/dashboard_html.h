...
"</td></tr>"
"<tr><td>SRP Server fallback (Thread DNS-SD)<br><span style='font-size:.8em;color:#666'>"
"ON = run a fallback SRP server ONLY when no border router is present, so local bindings survive a TBR/HA outage; yields to any TBR automatically.<br>"
"OFF = always rely on an external TBR (e.g. Google Nest) for discovery (default).</span></td>"
...
"<tr><td>Power meter (BL0942)</td><td class=hw-val id=hw-power>-</td></tr>"
"</table>"
...
"      var sr=document.getElementById('hw-srp');"
"      if(sr){sr.childNodes[0].textContent=d.srp_mode||'N/A';}"
...
"}"
"function toggleSRP(){"
"  if(!confirm('Toggle SRP Server mode? Device will restart.'))return;"
"  var x=new XMLHttpRequest();"
"  x.onload=function(){"
"    if(x.status==200){document.getElementById('hw-msg').innerHTML="
"      '<span class=ok>SRP mode changed. Restarting...</span>'}"
"    else{document.getElementById('hw-msg').innerHTML="
"      '<span class=err>Failed to toggle SRP mode</span>'}"
"  };"
"  x.onerror=function(){document.getElementById('hw-msg').innerHTML="
"    '<span class=err>Connection error</span>'};"
